#  evaluater for P2RNet.
import os
import numpy as np
import torch

from models.evaluating import BaseEvaluater
...
class Evaluater(BaseEvaluater, Trainer):
    ...
    def interpolate(self, room_idx_1, room_idx_2, interval, room_type_idx, start_deform=False):
        ...
        if self.cfg.config.distributed.use_ddp:
            ...
            latent_z = self.latent_input.interpolate(room_idx_1, room_idx_2, interval)
            est_data = self.generator.generate(latent_z, {"room_type_idx": room_type_idx}, start_deform=start_deform, self_end=True,
                                                    output_render=False)
        return est_data
    ...
    def generate(self, room_type_idx, start_deform=False):
        ...
        if self.cfg.config.distributed.use_ddp:
            ...
            latent_z = self.latent_input.sample_latent()
            est_data = self.generator.generate(latent_z, {"room_type_idx": room_type_idx}, start_deform=start_deform, self_end=True,
                                                    output_render=False)
        return est_data
    ...
    def visualize_interp(self, phase, output_filename, est_data):
        ...
        for batch_id, output in enumerate(est_data):
            ...
            dump_file = os.path.join(dump_dir, "{:s}/{:s}_{:d}.npz".format(room_type, output_filename, batch_id))
    ...
    def visualize_step(self, phase, iter, gt_data, est_data, dump_dir=None):
        ...
        for batch_id, output in enumerate(est_data):
            ...
            if self.cfg.config.start_deform:
                ...
                silhouettes = [None]
                seg_obj_ids= [None]
            dump_file = os.path.join(dump_dir, "{:s}_{:s}.npz".format(self.cfg.config.data.split_type, gt_data["sample_name"][batch_id][0]))
            np.savez(dump_file,
                     category_ids=category_ids[0],
                     centers=centers[0],
                     sizes=sizes[0],
