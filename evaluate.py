...
from utils.net_utils import load_device, load_model, load_trainer, load_evaluater, load_dataloader
...
class Evaluate(object):
    def __init__(self, cfg):
        ...
        cfg.info("Data save path: {:s}".format(os.getcwd()))
        ...
        self.split = cfg.config.eval.finetune_split
        n_views_for_finetune = cfg.config.eval.n_views_for_finetune
        ...
        for net_type, subnet in self.net.items():
            ...
            self.cfg.info("{:s} is frozen.".format(net_type))
        ...
        cfg.info("Loading method trainer and evaluater.")
        self.subtrainer = load_trainer(cfg=cfg, net=self.net, optimizer=self.optimizer, device=device)
        self.subevaluater = load_evaluater(cfg=cfg, net=self.net, device=device)

        """ Output network size """
        self.subevaluater.show_net_n_params()
    ...
    def evaluate_epoch(self, matching_by_rooms:Dict=dict(), **kwargs):
        """ Evaluate """
        ...
        for iter, data in enumerate(dataloader):
            ...
            loss, est_data = self.subevaluater.evaluate_step(data, start_deform=self.cfg.config.start_deform,
                                                      pred_gt_matching=pred_gt_matching)
            # visualize intermediate results.
            if self.cfg.config.generation.dump_results:
                self.subevaluater.visualize_step(phase, iter, data, est_data)
        ...
        for loss_name, loss_value in loss_recorder.loss_recorder.items():
            self.cfg.info("Currently the last {:s} loss ({:s}) is: {:f}".format(phase, loss_name, loss_value.avg))
        self.cfg.info("=" * 100)
    ...
    def run(self):
        ...
        # ---------------------------------------------------------------------------------------
        start_epoch = 0
        total_epochs = self.cfg.config.eval.epochs
        ...
        for epoch in range(start_epoch, total_epochs):
            self.cfg.info("-" * 100)
            self.cfg.info("Epoch ({:d}/{:s}):".format(epoch, total_epochs - 1))
            ...
            self.cfg.info("Epoch ({:d}/{:s}) Time elapsed: ({:f}).".format(epoch, total_epochs - 1, time() - epoch_start))
            ...
            if epoch == 0 or eval_loss < min_eval_loss:
                ...
                for loss_name, loss_value in eval_loss_recorder.items():
                    self.cfg.info("Currently the best val loss ({:s}) is: {:f}".format(loss_name, loss_value.avg))
                self.cfg.info("=" * 100)

        self.cfg.info("Finetuning time elapsed: ({:f}).".format(time() - finetune_start))

        """ Start to evaluate """
        self.cfg.info("Start to evaluate.")
        # ---------------------------------------------------------------------------------------
        # set mode
        self.subevaluater.set_mode("eval")
        start = time()
        with torch.no_grad():
            self.evaluate_epoch(matching_by_rooms=matching_by_rooms)
        self.cfg.info("evaluation time elapsed: ({:f}).".format(time() - start))
        # ---------------------------------------------------------------------------------------
        wandb.finish()
        self.cfg.info("evaluating finished.")