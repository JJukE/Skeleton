...
def multi_proc_run(num_proc, func, func_args=(), fun_kwargs={}):
    ...
    for i in range(num_proc):
        p_i = mp.Process(
            target=run,
            args=(i, num_proc, port, error_queue, func, func_args, fun_kwargs)
        )
        ps.append(p_i)
        p_i.start()
...
def initiate_environment(config):
    if config.use_randomseed:
        config.seed = random.randint(1, 10000)
        config.info("use_randomseed: {}, seed: {}".format(config.use_randomseed, config.seed))
    ...
    if torch.cuda.is_available():
        ...
        torch.backends.cudnn.benchmark = config.device.cudnn_benchmark
        torch.backends.cudnn.deterministic = config.device.cudnn_deterministic
